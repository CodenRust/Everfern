...
exports.detectToolCallLoop = detectToolCallLoop;
...
function hashToolOutcome(result, error) {
    if (error !== undefined)
        return `error:${digest(getFuzzyString(String(error)))}`;
 ...
 */
function detectToolCallLoop(history, toolName, params, config = {}) {
 ...
 */
function recordToolCall(history, toolName, params, config = {}) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    history.push({
        toolName,
 ...
 */
function recordToolOutcome(history, toolName, params, result, error) {
    const argsHash = hashToolCall(toolName, params);
    const resultHash = hashToolOutcome(result, error);
    for (let i = history.length - 1; i >= 0; i--) {
        const call = history[i];
        if (!call || call.toolName !== toolName || call.argsHash !== argsHash)
            continue;
