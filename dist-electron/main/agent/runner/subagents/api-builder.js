...
exports.resetAPIBuilder = resetAPIBuilder;
/**
...
class APIBuilder {
     ...
     */
    createRESTfulEndpoints(resource, operations) {
        const endpoints = [];
...
export const notFoundHandler = (req: Request, res: Response) => {
  res.status(404).json({
     ...
     */
    createAuthenticationSystem(config) {
        const files = [];
        switch (config.type) {
            case 'jwt':
     ...
     */
    createAuthorizationMiddleware(roles) {
        return `
import { Request, Response, NextFunction } from 'express';
...
export const requireOwnership = (resourceIdParam: string = 'id') => {
  return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
     ...
     */
    async generateDocumentation(spec) {
        const openApiSpec = {
            openapi: '3.0.0',
            info: {
                title: spec.name,
...
export const corsMiddleware = cors(corsOptions);`;
    ...
    async createMiddlewareFiles(spec) {
        return [
    ...
    async createValidationSchemas(endpoints) {
        const schemaFiles = [];
    ...
    async createErrorHandling(errorConfig) {
        return ['src/middleware/errorHandler.ts', 'src/utils/errors.ts'];
    }
    createJWTAuthentication(config) {
        return 'src/middleware/jwt-auth.ts';
    }
    createOAuthAuthentication(config) {
...
export const validateBody = (schema: z.ZodSchema) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    formatOpenAPIResponses(responses) {
        const formatted = {};
 ...
 */
let apiBuilderInstance = null;
/**
 * Get or create the API builder singleton
 */
function getAPIBuilder() {
