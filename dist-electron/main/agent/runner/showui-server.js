...
exports.isShowUIAlive = isShowUIAlive;
exports.ensureShowUIServer = ensureShowUIServer;
exports.killShowUIServer = killShowUIServer;
 ...
 */
async function ensureShowUIServer(onLog = () => { }) {
    // Already running?
    if (await isShowUIAlive())
        return true;
    // Already booting?
    if (bootingPromise)
    ...
    bootingPromise = (async () => {
        try {
            ...
            showuiProc = (0, child_process_1.spawn)('wsl', ['bash', '-c', cmd], { shell: false, detached: false });
            showuiProc.stdout?.on('data', (d) => d.toString().split('\n').filter(Boolean).forEach(l => onLog(`[ShowUI] ${l}`)));
            showuiProc.stderr?.on('data', (d) => d.toString().split('\n').filter(Boolean).forEach(l => onLog(`[ShowUI:err] ${l}`)));
            showuiProc.on('exit', (code) => {
                if (code !== 0)
                    onLog(`[ShowUI] Process exited with code ${code}`);
                showuiProc = null;
                bootingPromise = null;
            });
