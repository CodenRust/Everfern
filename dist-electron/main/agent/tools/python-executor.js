...
const registry_1 = require("./terminal/registry");
const os = __importStar(require("os"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const analysis_session_1 = require("../sessions/analysis-session");
/**
 * Python Executor
...
`;
     ...
     */
    async execute(code, timeout = this.defaultTimeout) {
     ...
     */
    async executeWithProgress(code, onProgress) {
     ...
     * @returns Execution result
     */
    async executeInSession(code, sessionId, stepLabel) {
        const result = await this.execute(code);
        const sessionManager = (0, analysis_session_1.getAnalysisSessionManager)();
     ...
     */
    detectCommonRuntimeError(errorOutput) {
        const lineMatch = errorOutput.match(/line (\d+)/);
        const line = lineMatch ? parseInt(lineMatch[1], 10) : undefined;
     ...
     */
    generateLibraryMissingSuggestions(moduleName) {
        const suggestions = [
     ...
     * @returns Array of actionable suggestions
     */
    generateSyntaxErrorSuggestions(errorOutput) {
        const suggestions = [];
