...
exports.SiteSelectorImpl = void 0;
 ...
 */
class SiteSelectorImpl extends intelligent_site_selection_base_1.BaseSiteSelector {
    urlClassifier;
    relevanceEngine;
    contentAnalyzer;
    scoringWeights;
    feedbackHistory = [];
    constructor(aiClient, config, urlClassifier, relevanceEngine, contentAnalyzer) {
    ...
    async evaluateSite(url, context) {
        ...
        try {
            ...
            return {
                ...
                estimatedValue: 0,
                riskAssessment: intelligent_site_selection_1.RiskLevel.HIGH
            };
        }
    }
    /**
    ...
    shouldVisitSite(evaluation) {
        ...
        if (evaluation.riskAssessment === intelligent_site_selection_1.RiskLevel.HIGH) {
            return false;
        }
     ...
     */
    async calculateScoringFactors(url, context, urlClassification) {
        const factors = {
    ...
    calculateWeightedScore(factors) {
        const weightedSum = factors.urlPatterns * this.scoringWeights.urlPatterns +
     ...
     */
    generateReasoningFactors(factors, urlClassification) {
        const reasoningFactors = [];
        // URL Pattern Analysis
        reasoningFactors.push(this.createReasoningFactor('URL Patterns', this.scoringWeights.urlPatterns, factors.urlPatterns, `URL classification: ${urlClassification.category}, matched ${urlClassification.patterns.length} patterns`));
        // Keyword Matching
        reasoningFactors.push(this.createReasoningFactor('Keyword Match', this.scoringWeights.keywordMatch, factors.keywordMatch, `Keyword relevance score: ${(factors.keywordMatch * 100).toFixed(1)}%`));
        // Domain Authority
        reasoningFactors.push(this.createReasoningFactor('Domain Authority', this.scoringWeights.domainAuthority, factors.domainAuthority, `Domain authority assessment: ${(factors.domainAuthority * 100).toFixed(1)}%`));
        // Content Indicators
        reasoningFactors.push(this.createReasoningFactor('Content Indicators', this.scoringWeights.contentIndicators, factors.contentIndicators, `Content quality indicators: ${(factors.contentIndicators * 100).toFixed(1)}%`));
        // Contextual Fit
        reasoningFactors.push(this.createReasoningFactor('Contextual Fit', this.scoringWeights.contextualFit, factors.contextualFit, `Alignment with research context: ${(factors.contextualFit * 100).toFixed(1)}%`));
        // Uniqueness
        reasoningFactors.push(this.createReasoningFactor('Uniqueness', this.scoringWeights.uniqueness, factors.uniqueness, `Content uniqueness score: ${(factors.uniqueness * 100).toFixed(1)}%`));
        // Risk Assessment
        reasoningFactors.push(this.createReasoningFactor('Risk Assessment', this.scoringWeights.riskAssessment, factors.riskAssessment, `Risk level: ${urlClassification.riskLevel}`));
        return reasoningFactors;
    }
    /**
     * Calculate keyword matching score
     */
    calculateKeywordMatch(url, keywords) {
        if (keywords.length === 0)
            return 0.5; // Neutral score for no keywords
     ...
     */
    calculateContentIndicators(url) {
        const urlLower = url.toLowerCase();
     ...
     */
    calculateContextualFit(url, context) {
     ...
     */
    async calculateUniqueness(url, context) {
    ...
    calculateRiskScore(riskLevel) {
        switch (riskLevel) {
            case intelligent_site_selection_1.RiskLevel.LOW:
                return 0.9;
            case intelligent_site_selection_1.RiskLevel.MEDIUM:
                return 0.6;
            case intelligent_site_selection_1.RiskLevel.HIGH:
                return 0.1;
            default:
                return 0.5;
        }
     ...
     */
    calculateConfidence(factors, urlClassification) {
        ...
        const avgPatternConfidence = urlClassification.patterns.length > 0
            ? urlClassification.patterns.reduce((sum, p) => sum + p.confidence, 0) / urlClassification.patterns.length
            : 0.5;
     ...
     */
    estimateValue(relevanceScore, context) {
     ...
     */
    generateRankingReasoning(evaluation, rank) {
        const topFactors = evaluation.reasoningFactors
            .sort((a, b) => (b.weight * b.contribution) - (a.weight * a.contribution))
            .slice(0, 3);
        const factorDescriptions = topFactors.map(factor => `${factor.factor} (${(factor.contribution * 100).toFixed(1)}%)`).join(', ');
     ...
     * Adjust scoring weights based on feedback
     */
    adjustScoringWeights(successfulDecisions, unsuccessfulDecisions) {
    ...
    normalizeWeights() {
        ...
        if (totalWeight > 0) {
            for (const key in this.scoringWeights) {
                this.scoringWeights[key] /= totalWeight;
            }
        }
    }
    /**
     ...
     */
    generatePatternLearningData(successfulDecisions, unsuccessfulDecisions) {
...
exports.SiteSelectorImpl = SiteSelectorImpl;
