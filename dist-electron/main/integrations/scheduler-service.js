...
class SchedulerService {
        ...
        try {
            ...
            for (const task of tasks) {
                ...
                if (task.endsAt && new Date() > new Date(task.endsAt)) {
                    ...
                    await scheduled_tasks_1.scheduledTasksStore.save({ id: task.id, enabled: false });
                    continue;
                }
    ...
    shouldRun(task, now) {
        ...
        if (!task.nextRun && !task.lastRun) {
            ...
            scheduled_tasks_1.scheduledTasksStore.updateRunTimes(task.id, '', nextRun.toISOString());
            return false;
        }
        return false;
    }
     ...
     */
    static calculateNextRun(cron, lastRun, startsAt) {
        const now = new Date();
        let baseDate;
        if (lastRun) {
    ...
    async runTask(task) {
        ...
        try {
            ...
            const historyStore = new history_1.ChatHistoryStore();
            ...
            const client = (0, ai_client_1.getPooledAIClient)({ provider: 'ollama', model: 'llama3' });
            ...
            await scheduled_tasks_1.scheduledTasksStore.updateRunTimes(task.id, now.toISOString(), nextRun.toISOString());
