...
User Input: "${userInput}"`;
  ...
  const timeoutPromise = new Promise<never>((_, reject) => 
    setTimeout(() => reject(new Error('Triage AI call timed out')), 30000)
  );
  ...
  try {
    ...
    return {
      intent: (data.intent || 'unknown') as IntentType,
 ...
 */
export function classifyIntentHeuristic(userInput: string): IntentClassification {
