...
class IntentCache {
  ...
  private key(input: string, historyLength: number): string {
  ...
  get(input: string, historyLength: number): IntentClassification | null {
  ...
  set(input: string, historyLength: number, classification: IntentClassification): void {
...
export function classifyIntentFallback(userInput: string, history: any[] = []): IntentClassification {
  ...
  if (normalized.length < 20 && affirmatives.some(a => normalized === a || normalized.startsWith(a + ' '))) {
    ...
    const userMsgs = (history || []).filter((m: any) => m.role === 'user' || m.type === 'human');
