...
export type ElementType =
  ...
  | 'dropdown' | 'tab' | 'menu' | 'icon' | 'text' | 'image' | 'other';

export interface SoMElement {
  id: number;
  label: string;         // Short human-readable label, e.g. "Submit", "Search box"
  type: ElementType;
  x: number;             // Center x in screenshot pixels
  y: number;             // Center y in screenshot pixels
  x_pct: number;
  y_pct: number;
...
export interface SoMResult {
  elements: SoMElement[];
  annotatedB64: string;  // Screenshot with numbered overlay boxes
  screenHash: number;    // For cache invalidation
}
...
const BACKEND_FAILURES = new Map<string, number>();

function isBackendHealthy(key: string): boolean {
  const failedAt = BACKEND_FAILURES.get(key);
  if (!failedAt) return true;
  if (Date.now() - failedAt > BACKEND_COOLDOWN_MS) { BACKEND_FAILURES.delete(key); return true; }
  return false;
}

function markBackendFailed(key: string): void {
...
// ── Locate result cache (per query × screen) ──────────────────────────────────

interface CacheEntry { result: GroundingResult; ts: number }
const LOCATE_CACHE = new Map<string, CacheEntry>();
const CACHE_TTL_MS = 4_000;

function locateCacheKey(query: string, b64: string): string {
  return query.toLowerCase().trim() + ':' + cheapHash(b64).toString(16);
}

function locateCacheGet(key: string): GroundingResult | null {
  const entry = LOCATE_CACHE.get(key);
  if (!entry) return null;
  if (Date.now() - entry.ts > CACHE_TTL_MS) { LOCATE_CACHE.delete(key); return null; }
  return entry.result;
}

function locateCacheSet(key: string, result: GroundingResult): void {
  const now = Date.now();
  for (const [k, v] of LOCATE_CACHE) if (now - v.ts > CACHE_TTL_MS) LOCATE_CACHE.delete(k);
  LOCATE_CACHE.set(key, { result, ts: now });
}

// ── SoM element cache (per screen hash) ───────────────────────────────────────

interface SoMCacheEntry { result: SoMResult; ts: number }
const SOM_CACHE = new Map<number, SoMCacheEntry>();
const SOM_CACHE_TTL_MS = 3_000;

function somCacheGet(hash: number): SoMResult | null {
  const entry = SOM_CACHE.get(hash);
  if (!entry) return null;
  if (Date.now() - entry.ts > SOM_CACHE_TTL_MS) { SOM_CACHE.delete(hash); return null; }
  return entry.result;
}

function somCacheSet(hash: number, result: SoMResult): void {
  const now = Date.now();
  for (const [k, v] of SOM_CACHE) if (now - v.ts > SOM_CACHE_TTL_MS) SOM_CACHE.delete(k);
...
const REGION_KEYWORDS: Array<{ patterns: RegExp[]; getRegion: (w: number, h: number) => Region }> = [
  {
...
function getRegionHint(query: string, imgW: number, imgH: number): Region | null {
  for (const hint of REGION_KEYWORDS) {
    if (hint.patterns.some(p => p.test(query))) return hint.getRegion(imgW, imgH);
  }
  return null;
}

function cropImage(b64: string, region: Region) {
  ...
  const round32 = (v: number) => Math.max(32, Math.floor(v / 32) * 32);
  const finalW = round32(w);
  const finalH = round32(h);

  const finalB64 = nativeImage.createFromBuffer(cropped.toJPEG(90))
 ...
 */
async function prepareImageForQwen(b64: string, w: number, h: number): Promise<{ b64: string; rw: number; rh: number }> {
  ...
  const round32 = (v: number) => Math.max(SCALE_32, Math.floor(v / SCALE_32) * SCALE_32);
 ...
 */
async function annotateSoM(b64: string, elements: SoMElement[], w: number, h: number): Promise<string> {
  const PILL_H = 20, PAD = 6, R = 4;

  const svgParts: string[] = [`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}">`];
  ...
  for (const el of elements) {
    ...
    svgParts.push(
      `<circle cx="${el.x}" cy="${el.y}" r="5" fill="${color}" opacity="0.85"/>`,
      `<circle cx="${el.x}" cy="${el.y}" r="5" fill="none" stroke="white" stroke-width="1.5"/>`,
      // Pill background
...
function buildSoMPrompt(w: number, h: number): string {
...
function buildGroundingPrompt(query: string, w: number, h: number, attempt: number): string {
...
function parseGroundingJSON(raw: string, imgW: number, imgH: number) {
  ...
  try {
    ...
    if (parsed.x_pct !== undefined) { xNorm = parsed.x_pct / 100; yNorm = parsed.y_pct / 100; }
    else { xNorm = parsed.x / 1000; yNorm = parsed.y / 1000; }
...
function parseSoMJSON(raw: string, imgW: number, imgH: number): SoMElement[] {
  try {
    ...
    for (const item of arr) {
      if (typeof item.x_pct !== 'number') continue;
      ...
      elements.push({
        ...
        type: (item.type as ElementType) ?? 'other',
...
async function groundViaShowUILocal(
  ...
  const ready = await ensureShowUIServer((l) => console.log(l));
  if (!ready) throw new Error('ShowUI server failed to start');
  ...
  if (!resp.ok) throw new Error(`ShowUI ${resp.status}: ${await resp.text()}`);

  const data = await resp.json();
  const rawArr = data?.data?.[0];
  if (!Array.isArray(rawArr) || rawArr.length < 2) return { found: false, x: 0, y: 0, confidence: 0, source: 'showui-local' };

  const rx = Number(rawArr[0]), ry = Number(rawArr[1]);
  if (isNaN(rx) || isNaN(ry)) return { found: false, x: 0, y: 0, confidence: 0, source: 'showui-local' };
...
async function groundViaOllama(
  b64: string, imgW: number, imgH: number, query: string,
  baseUrl: string, modelName: string, apiKey: string | undefined,
  attempt: number, isSlim: boolean,
): Promise<GroundingResult> {
  const source = isSlim ? 'ollama-slim' : (apiKey ? 'ollama-cloud' : 'ollama-maiui') as GroundingResult['source'];
...
async function detectViaOllama(
  b64: string, imgW: number, imgH: number,
  baseUrl: string, modelName: string, apiKey: string | undefined,
): Promise<SoMElement[]> {
  const resp = await fetch(baseUrl.replace(/\/$/, '') + '/api/chat', {
    method: 'POST',
  ...
  if (!resp.ok) throw new Error(`Ollama (SoM) ${resp.status}: ${await resp.text()}`);
...
async function detectViaFallback(
  b64: string, imgW: number, imgH: number, client: AIClient,
): Promise<SoMElement[]> {
...
function withTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
  return Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`[Grounding] ${label} timed out after ${ms}ms`)), ms)
    ),
  ]);
}

async function groundViaFallback(
  b64: string, imgW: number, imgH: number, query: string,
  client: AIClient, attempt: number,
): Promise<GroundingResult> {
  const response = await withTimeout(
    client.chat({
      model: client.model,
...
export class GroundingEngine {
  ...
  get backendName(): string {
    if (this.config.showuiUrl) return `ShowUI (${this.config.showuiUrl})`;
    if (this.config.ollamaBaseUrl) return `Ollama (${this.config.ollamaModel ?? 'qwen3-vl:2b'})`;
    return 'Vision Fallback';
  }

  async checkShowUIHealth(): Promise<boolean> {
    if (!this.config.showuiUrl) return false;
    try {
  ...
  async detectElements(
    ...
    if (elements.length === 0 && this.config.fallbackClient) {
      try { elements = await detectViaFallback(workB64, workW, workH, this.config.fallbackClient); }
      catch (err) { console.warn('[Grounding] Fallback SoM failed:', err); }
    }
    ...
    const result: SoMResult = { elements, annotatedB64, screenHash: hash };
   ...
   */
  getElementById(b64: string, id: number): SoMElement | null {
    const hash = cheapHash(b64);
    const cached = somCacheGet(hash);
   ...
   */
  matchElementByText(b64: string, query: string): GroundingResult | null {
    const hash = cheapHash(b64);
    const cached = somCacheGet(hash);
    if (!cached || cached.elements.length === 0) return null;
    ...
    if (!el) el = cached.elements.find(e => e.label.toLowerCase().startsWith(q));
    if (!el) el = cached.elements.find(e => e.label.toLowerCase().includes(q));
    // Reverse: query contains the label (e.g. query "click the Submit button" → label "Submit")
    if (!el) el = cached.elements.find(e => q.includes(e.label.toLowerCase()));
    ...
    for (let attempt = 1; attempt <= MAX_LOCATE_ATTEMPTS; attempt++) {
      ...
      if (attempt === 1) {
        ...
        if (hint) {
          ...
          workB64 = crop.croppedB64; workW = crop.cropW; workH = crop.cropH;
          offsetX = crop.offsetX; offsetY = crop.offsetY;
      ...
      if (attempt === 3) {
        ...
        workB64 = prepped.b64; workW = prepped.rw; workH = prepped.rh;
      }
      ...
      if (this.config.showuiUrl && isBackendHealthy('showui-local')) {
        candidates.push(
          groundViaShowUILocal(workB64, workW, workH, query, this.config.showuiUrl)
      ...
      if (this.config.ollamaBaseUrl) {
        ...
        if (isBackendHealthy(backendKey)) {
          candidates.push(
            groundViaOllama(workB64, workW, workH, query, this.config.ollamaBaseUrl, model, apiKey, attempt, isSlim)
              .then(r => r.found ? r : null)
      ...
      if (this.config.fallbackClient && (!hasOtherBackend || attempt === MAX_LOCATE_ATTEMPTS)) {
        candidates.push(
          groundViaFallback(workB64, workW, workH, query, this.config.fallbackClient, attempt)
            .then(r => r.found ? r : null)
            .catch(err => { console.warn('[Grounding] Fallback error:', err); return null; })
        );
      }

      if (candidates.length === 0) { console.warn('[Grounding] No backends.'); break; }
      ...
      if (winner) {
        ...
        const final: GroundingResult = { ...winner, x: finalX, y: finalY, attempts: attempt };

        console.log(`[Grounding] ✅ (${finalX}, ${finalY}) via ${winner.source} [atmt ${attempt}, conf ${Math.round(winner.confidence * 100)}%]`);
        if (winner.confidence >= 0.85) locateCacheSet(ck, final);
        return final;
      }
...
async function raceToFound<T>(promises: Promise<T | null>[]): Promise<T | null> {
  return new Promise((resolve) => {
    ...
    for (const p of promises) {
      ...
      }).catch(() => { if (++settled === promises.length) resolve(null); });
    }
  });
}
...
/**
 * ElementData from Chrome extension
 * Represents a captured DOM element with its properties and location
 */
export interface ElementData {
  selector: string;
  boundingRect: DOMRect | { left: number; top: number; right: number; bottom: number; width: number; height: number };
  tagName: string;
  textContent: string;
...
/**
 * Enhanced grounding result with method information
 */
export interface VisualGroundingResult extends GroundingResult {
...
/**
 * Configuration for enhanced grounding with fallback capabilities
 */
export interface EnhancedGroundingConfig extends GroundingConfig {
...
/**
 * Learning record for successful visual interactions
 */
 ...
 */
export class EnhancedGroundingEngine extends GroundingEngine {
  public enhancedConfig: EnhancedGroundingConfig;
  private extensionElements: Map<string, ElementData> = new Map();
  private lastExtensionUpdate: number = 0;
  private extensionUpdateTTL: number = 5000; // 5 seconds
  private learningLog: VisualInteractionLearning[] = [];
  private maxLearningLogSize: number = 1000;
  private selectorOptimizations: Map<string, string> = new Map(); // Maps queries to optimized selectors

  constructor(config: EnhancedGroundingConfig) {
    super(config);
    this.enhancedConfig = {
   ...
   */
  updateExtensionElements(elements: ElementData[]): void {
   ...
   */
  private isExtensionDataFresh(): boolean {
  ...
  private matchExtensionElements(query: string): VisualGroundingResult | null {
    ...
    const q = query.toLowerCase().trim();
    let bestMatch: { selector: string; el: ElementData; confidence: number } | null = null;
  ...
  private recordLearning(
    ...
    const learning: VisualInteractionLearning = {
      query,
      method,
      selector,
      confidence,
 ...
 */
export function createGroundingEngine(config: EnhancedGroundingConfig | GroundingConfig): GroundingEngine | EnhancedGroundingEngine {
  if ('enableSoMDetection' in config || 'fallbackToVisual' in config) {
    return new EnhancedGroundingEngine(config as EnhancedGroundingConfig);
  }
