 ...
 */
export const createSwarmOrchestratorNode = (
  runner: AgentRunner,
  ...
  return async (state: GraphStateType): Promise<Partial<GraphStateType>> => {
    ...
    if (!plan || plan.steps.length === 0) {
      ...
      return { taskPhase: 'evaluating' as const };
    }
    ...
    if (!hasSpawnedSubagents) {
        ...
        if (parallelSteps.length === 0) {
            ...
            return { taskPhase: 'brain' as const };
        }
    ...
    if (hasSpawnedSubagents) {
        const ids = spawnedAgents.map((s: any) => s.agentId);
        ...
        const statusMap = ids.map((id: string) => {
            ...
            const spawnedInfo = spawnedAgents.find((s: any) => s.agentId === id);
        ...
        if (!allDone) {
            const runningCount = statusMap.filter((s: any) => s.status === 'running' || s.status === 'pending').length;
...
function mapToolToAgentType(tool?: string): any {
    if (!tool) return 'generic';
    const t = tool.toLowerCase();
    if (t.includes('web') || t === 'navis') return 'web-explorer';
    if (t.includes('write') || t.includes('edit') || t.includes('bash') || t.includes('command')) return 'coding-specialist';
    if (t === 'computer_use') return 'computer-use';
    if (t === 'analyze') return 'data-analyst';
    return 'generic';
}
