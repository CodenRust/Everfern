...
import { getFrameworkExpert, type FrameworkTemplate, type DirectoryNode } from './framework-expert';
...
export class ProjectScaffolder {
  private frameworkExpert = getFrameworkExpert();
  ...
  async scaffold(options: ScaffoldOptions): Promise<ScaffoldResult> {
    ...
    try {
      ...
      const structureFiles = this.generateDirectoryStructure(
   ...
   * Generate package.json
   */
  private generatePackageJson(template: FrameworkTemplate, options: ScaffoldOptions): string {
    const packageJson = {
   ...
   * Generate README.md
   */
  private generateReadme(template: FrameworkTemplate, options: ScaffoldOptions): string {
...
`;
   ...
   * Generate .gitignore
   */
  private generateGitignore(template: FrameworkTemplate): string {
...
`;
   ...
   */
  private generateEnvTemplate(template: FrameworkTemplate): string {
...
`;
   ...
   */
  private generateDockerFiles(template: FrameworkTemplate, options: ScaffoldOptions): FileToCreate[] {
...
out`,
   ...
   */
  private generateBackendDockerfile(template: FrameworkTemplate): string {
...
`;
   ...
   */
  private generateFrontendDockerfile(template: FrameworkTemplate): string {
    if (template.name.includes('Next.js')) {
      return `FROM node:18-alpine AS base
...
`;
   ...
   */
  private generateDockerCompose(template: FrameworkTemplate, options: ScaffoldOptions): string {
    return `version: '3.8'
...
`;
   ...
   */
  private generateCIFiles(template: FrameworkTemplate, options: ScaffoldOptions): FileToCreate[] {
    const files: FileToCreate[] = [];
   ...
   */
  private generateGitHubWorkflow(template: FrameworkTemplate, options: ScaffoldOptions): string {
    return `name: CI/CD
...
`;
   ...
   * Generate setup instructions
   */
  private generateInstructions(template: FrameworkTemplate, options: ScaffoldOptions): string[] {
    const pm = options.packageManager || 'npm';
    const installCmd = pm === 'npm' ? 'npm install' : pm === 'yarn' ? 'yarn' : 'pnpm install';
    const devCmd = pm === 'npm' ? 'npm run dev' : pm === 'yarn' ? 'yarn dev' : 'pnpm dev';
...
// Singleton instance
let scaffolderInstance: ProjectScaffolder | null = null;
