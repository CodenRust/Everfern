  ...
  private absoluteXy(
    ...
    if (displayW && displayH) {
      if (x <= 1000 && y <= 1000) {
        ...
        console.log(
          `[Coordinate Transform] relative input=(${x}, ${y}) physical_target=(${displayW}x${displayH}) → abs=(${absX}, ${absY})`
        );
        return [absX, absY];
      }
      ...
      if (imageW && imageH) {
        ...
        console.log(
          `[Coordinate Transform] pixel input=(${x}, ${y}) image_size=(${imageW}x${imageH}) scale=(${scaleX.toFixed(2)}, ${scaleY.toFixed(2)}) → abs=(${absX}, ${absY})`
        );
        return [absX, absY];
      }
    }
...
class ComputerUseAgent {
  ...
  async run(onUpdate?: (msg: string) => void): Promise<{ finalAnswer: string, lastScreenshot?: string }> {
    ...
    for (let step = 1; step <= this.maxTurns; step++) {
      ...
      const response = await this.client.chat({
        ...
        tools: [COMPUTER_USE_TOOL_SPEC as any],
      });
      ...
      for (const toolCall of toolCalls) {
        ...
        onUpdate?.(`Sub-Agent executing ${args.action as string}...`);
...
export function createComputerUseTool(
  ...
  vlm?: { provider: string; model: string; baseUrl?: string; apiKey?: string },
  ...
  const subAgentClient = vlm ? new AIClient({
    provider: (vlm.provider as any) || 'openai',
    apiKey: vlm.apiKey,
    baseUrl: vlm.baseUrl,
    model: vlm.model
  }) : originalClient;
