...
import { createIntelligentSiteSelection } from './intelligent-site-selection-factory';
 ...
 */
export class IntelligentScoringSystem {
    ...
    try {
      ...
      if (heuristicAnalysis.relevanceScore < 30) {
        this.logDecision(
          ...
          `Heuristic score too low: ${heuristicAnalysis.fastRejectReasons?.join(', ')}`
        );
      ...
      if (heuristicAnalysis.relevanceScore >= 50) {
        const deepAnalysis = await this.intelligentSystem.contentAnalyzer.performDeepAnalysis(
          content,
          this.researchContext
        );
   ...
   */
  async evaluateSiteForVisit(
    url: string,
    taskDescription: string,
    content?: PageContent
  ): Promise<{ shouldVisit: boolean; score: number; reasoning: string }> {
    if (!this.researchContext) {
      this.researchContext = {
        taskDescription,
        goals: this.extractGoals(taskDescription),
    ...
    try {
      ...
      this.logDecision(
        ...
        evaluation.reasoningFactors.map(f => f.explanation).join('; ')
      );
  ...
  async getNavigationRecommendations(
    ...
    taskDescription: string
  ): Promise<Array<{ url: string; score: number; reasoning: string }>> {
    if (!this.researchContext) {
      this.researchContext = {
        taskDescription,
        goals: this.extractGoals(taskDescription),
        keywords: this.extractKeywords(taskDescription),
        currentPhase: ResearchPhase.ANALYSIS,
        timeConstraints: { urgency: 'medium' },
        qualityRequirements: { minRelevanceScore: 40, requireMultipleSources: false, factVerificationLevel: 'basic' },
        previousFindings: []
      };
    ...
    try {
      ...
      const scoredLinks = await Promise.all(
        links.map(async (link) => {
          const score = await this.scoreUrlRelevanceIntelligent(
            link.href,
            taskDescription,
            link.text
          );
   ...
   */
  updateResearchContext(context: Partial<ResearchContext>): void {
    if (this.researchContext) {
   ...
   */
  private fallbackUrlScoring(url: string, taskDescription: string, linkText: string): number {
    ...
    keywords.forEach(kw => {
      if (u.includes(kw) || text.includes(kw)) score += 15;
    });

    // Penalize for irrelevant patterns
    if (u.includes('login') || u.includes('signup') || u.includes('auth')) score -= 30;
    if (u.includes('cookie') || u.includes('privacy') || u.includes('terms')) score -= 20;
    if (u.includes('.pdf') || u.includes('.jpg') || u.includes('.png')) score -= 25;

    // Boost for relevant patterns
    if (u.includes('pricing') || u.includes('plans') || u.includes('cost')) score += 20;
    if (u.includes('features') || u.includes('capabilities')) score += 15;
    if (u.includes('documentation') || u.includes('docs') || u.includes('api')) score += 15;
  ...
  private fallbackPageScoring(taskDescription: string, content: PageContent): number {
    ...
    // Check title
    if (content.title.toLowerCase().includes(task)) score += 20;

    // Check meta description
    if (content.metaDescription?.toLowerCase().includes(task)) score += 15;

    // Check headings
    const headingText = content.headings.join(' ').toLowerCase();
    if (headingText.includes(task)) score += 10;

    // Check for structured data
    if (content.structuredData) score += 10;

    // Check for pricing/features
    if (content.rawText.toLowerCase().includes('pricing')) score += 10;
    if (content.rawText.toLowerCase().includes('features')) score += 10;
   ...
   */
  private extractGoals(taskDescription: string): string[] {
    const goals: string[] = [];
 ...
 */
export function createIntelligentScoringSystem(
  aiClient: AIClient,
  config?: Partial<IntelligentSelectionConfig>,
  researchContext?: ResearchContext
 ...
 */
export async function scoreUrlRelevanceWithIntelligence(
  url: string,
  taskDescription: string,
  linkText: string,
  scoringSystem: IntelligentScoringSystem
): Promise<number> {
 ...
 */
export async function scorePageRelevanceWithIntelligence(
  taskDescription: string,
  content: PageContent,
  scoringSystem: IntelligentScoringSystem
): Promise<number> {
 ...
 */
export function addIntelligentFactToMemory(
  memory: EnhancedSharedResearchMemoryImpl,
  url: string,
  title: string,
  summary: string,
  ...
  const intelligentFact: IntelligentExtractedFact = {
    url,
    title,
    summary,
    relevanceScore,
