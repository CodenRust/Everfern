...
/**
 * Context-aware scoring weights that adapt based on research state
 */
 ...
 * Requirements: 4.1, 4.4, 4.5
 */
export class ContextAwareScoringEngine {
  ...
  private phaseWeights: Record<ResearchPhase, Partial<AdaptiveWeights>> = {
   ...
   * Requirements: 4.4
   */
  private adjustWeightsForKeywords(
    weights: AdaptiveWeights,
    keywords: string[]
  ): AdaptiveWeights {
    const adjusted = { ...weights };
  ...
  private normalizeWeights(weights: AdaptiveWeights): AdaptiveWeights {
    ...
    const normalized: AdaptiveWeights = {} as AdaptiveWeights;
   ...
   * Requirements: 4.1
   */
  calculatePhaseAwarenessScore(
    phase: ResearchPhase,
    contentType: string
  ): number {
    const phaseContentPreferences: Record<ResearchPhase, Record<string, number>> = {
      [ResearchPhase.DISCOVERY]: {
        'overview': 0.9,
        'product': 0.8,
        'features': 0.7,
   ...
   * Requirements: 4.5
   */
  calculateGapPrioritizationScore(
    contentType: string,
    gaps: ContentGap[]
  ): number {
  ...
  calculateContextAwareScore(
    ...
    for (const [factor, weight] of Object.entries(weights)) {
      const factorValue = (factors as any)[factor] || 0;
 ...
 */
export function createContextAwareScoringEngine(): ContextAwareScoringEngine {
