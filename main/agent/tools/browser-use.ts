...
import type { GroundingEngine } from '../runner/grounding';
...
export interface ExtensionMessage {
  type:
    | 'activate' | 'deactivate' | 'capture' | 'highlight' | 'interact' | 'get-state'
    | 'activate-extension' | 'deactivate-extension' | 'capture-elements'
    | 'highlight-element' | 'apply-shimmer' | 'remove-shimmer'
    | 'get-extension-state' | 'detect-playwright';
  payload?: any;
}
...
class ChromeExtensionAPI {
  ...
  setPage(page: any, extensionId: string) {
    this.page = page;
    this.extensionId = extensionId;
  }

  async sendMessage(message: ExtensionMessage): Promise<any> {
    if (!this.page || !this.extensionId) return { success: false };
    return this.page.evaluate(
      ({ extId, msg }: any) => {
        return new Promise((resolve) => {
          const win = window as any;
          if (!win.chrome?.runtime?.sendMessage) return resolve({ success: false });
          win.chrome.runtime.sendMessage(extId, msg, (response: any) => {
            resolve(response || { success: true });
          });
        });
      },
      { extId: this.extensionId, msg: message }
    ).catch(() => ({ success: false }));
  }
...
export interface BrowserAction {
  action: 'navigate' | 'goto_link' | 'click' | 'type' | 'fill' | 'select' | 'scroll' | 'scroll_to' | 'hover' | 'back' | 'forward' | 'extract' | 'wait' | 'done';
  url?: string;
  target?: string;
  label?: string;
  text?: string;
...
type ResearchPhase = 'PLAN' | 'SEARCH' | 'DEEP_DIVE' | 'SYNTHESIZE';
...
export interface BrowserUseResult {
  ...
  results: Array<{ task: string; summary: string; sourcesVisited: number }>;
  steps: number;
  error?: string;
}
...
async function humanPause(page: any, base = 300, jitter = 200) {
  await page.waitForTimeout(base + Math.floor(Math.random() * jitter));
}

// ─────────────────────────────────────────────────────────────────────────────
// Heuristic fast-path — instant decisions without AI
// ─────────────────────────────────────────────────────────────────────────────

interface FastPathResult {
  skip: boolean;       // bail on this page entirely
  reason?: string;
  autoExtract?: boolean; // extract immediately without AI
}

function heuristicFastPath(url: string, title: string, rawText: string): FastPathResult {
...
async function smartScroll(page: any): Promise<void> {
...
async function extractPageContent(page: any): Promise<PageContent> {
  return page.evaluate(() => {
    // Focus on interactive + heading elements only, max depth 5
    const buildDomTree = (root: Element, depth = 0): string => {
      if (depth > 5) return '';
      let tree = '';
    ...
    document.querySelectorAll('script[type="application/ld+json"]').forEach(s => {
      try { jsonLd.push(s.textContent || ''); } catch {}
    });
    ...
    const mainEl = document.querySelector('main, article, [role="main"]') as HTMLElement | null;
    ...
    return {
      ...
      headings: Array.from(document.querySelectorAll('h1,h2,h3,h4')).map(h => (h as any).innerText.trim()).filter(Boolean),
      paragraphs: Array.from(document.querySelectorAll('p')).map(p => (p as any).innerText.trim()).filter(t => t.length > 40).slice(0, 30),
      tables: Array.from(document.querySelectorAll('table')).map(t =>
        Array.from(t.querySelectorAll('tr')).map(r =>
          Array.from(r.querySelectorAll('td,th')).map(c => (c as any).innerText.trim()).join(' | ')
        )
      ),
      links: Array.from(document.querySelectorAll('a[href]'))
        .map(a => ({ text: (a as any).innerText.trim(), href: (a as any).href }))
...
// ─────────────────────────────────────────────────────────────────────────────
// Shared Research Memory (cross-tab knowledge sharing)
// ─────────────────────────────────────────────────────────────────────────────
...
class SharedResearchMemory {
  ...
  private queuedUrls = new Set<string>(); // O(1) dedup
  private urlQueue: Array<{ url: string; score: number }> = [];
  ...
  dequeueUrl(): string | undefined {
    ...
    if (item) this.queuedUrls.delete(item.url);
    return item?.url;
  }

  peekNextUrl(): string | undefined {
  ...
  getSummary(): string {
    ...
    if (this.facts.length === 1) {
      ...
      if (f.prices.length) parts.push(`Pricing: ${f.prices.join(', ')}`);
      if (f.ratings.length) parts.push(`Ratings: ${f.ratings.join(', ')}`);
      if (f.keyFacts.length) parts.push(f.keyFacts.join('. '));
    ...
    return this.facts.map(f => {
      ...
      if (f.prices.length) parts.push(`Pricing: ${f.prices.join(', ')}`);
      if (f.ratings.length) parts.push(`Ratings: ${f.ratings.join(', ')}`);
      if (f.keyFacts.length) parts.push(`Facts: ${f.keyFacts.join(' | ')}`);
      return parts.join(' — ');
    }).join('\n\n');
  }

  getFacts(): ExtractedFact[] { return [...this.facts]; }
 ...
 */
function scorePageRelevance(taskDescription: string, content: PageContent): number {
  ...
  if (/\/pricing|\/price|\/plans/.test(url)) score += 20;
  if (/\/features|\/product|\/overview/.test(url)) score += 15;
  if (/\/docs|\/documentation|\/guide/.test(url)) score += 10;
  if (/\/reviews|\/compare|\/vs/.test(url)) score += 15;
  if (/\/about/.test(url)) score += 5;

  // Penalty for clearly irrelevant pages
  if (/login|signin|signup|register|checkout|cart|cookie|privacy|terms|404|error/.test(url)) score -= 30;
  if (content.title.toLowerCase().includes('404') || content.title.toLowerCase().includes('not found')) score -= 50;
  if (content.rawText.length < 200) score -= 20; // Thin content

  // Bonus if prices or ratings detected (high-signal pages)
  if (content.prices && content.prices.length > 0) score += 15;
 ...
 */
function scoreUrlRelevance(url: string, taskDescription: string, linkText = ''): number {
  ...
  if (/\/pricing|\/price|\/plans/.test(u)) score += 35;
  if (/\/features|\/product|\/overview/.test(u)) score += 28;
  if (/\/docs|\/documentation/.test(u)) score += 22;
  if (/\/reviews|\/compare|\/vs/.test(u)) score += 28;
  if (/\/about/.test(u)) score += 10;
  if (/\/blog|\/news|\/article/.test(u)) score += 8;

  // Link text signals — "View details", "See pricing", "Learn more" etc.
  if (/view detail|see detail|more detail|full detail/.test(lt)) score += 30;
  if (/pricing|see price|view price|how much|cost/.test(lt)) score += 30;
  if (/features|capabilities|what.s included/.test(lt)) score += 25;
  if (/learn more|read more|find out more|discover/.test(lt)) score += 15;
  if (/review|rating|testimonial|what.s people say/.test(lt)) score += 20;
  if (/get started|try|demo|free trial/.test(lt)) score += 12;
  if (/documentation|docs|guide|how to/.test(lt)) score += 18;
  if (/compare|vs\b|versus|alternative/.test(lt)) score += 22;
  if (/about|overview|introduction/.test(lt)) score += 8;

  // Penalty for noise
  if (/login|signin|signup|register|checkout|cart|cookie|privacy|terms|tos|legal/.test(u)) score -= 60;
  if (/cdn\.|static\.|assets\.|img\.|images\./.test(u)) score -= 70;
  if (/\.(jpg|jpeg|png|gif|svg|pdf|zip|exe|dmg)$/.test(u)) score -= 90;
  if (/\?.*utm_/.test(u)) score -= 15;
  if (/#/.test(u)) score -= 5;
...
function discoverHighValueUrls(baseUrl: string, links: Array<{ text: string; href: string }>, taskDescription: string): Array<{ url: string; score: number }> {
...
function buildDirectUrls(baseUrl: string, taskDescription: string): Array<{ url: string; score: number }> {
  try {
...
async function planResearch(client: any, taskDescription: string): Promise<ResearchPlan> {
...
// ─────────────────────────────────────────────────────────────────────────────
// AI-powered page analysis — tool-calling approach, no rigid schema
// ─────────────────────────────────────────────────────────────────────────────
...
// Tool call result — minimal structure, AI writes what it wants in reasoning
interface ToolCall {
  tool: string;
...
async function analyzePageContent(
  ...
  content: PageContent,
  sharedMemory: SharedResearchMemory,
  plan?: ResearchPlan
...
interface AnalyzeAndDecideResult {
...
async function analyzeAndDecide(
  ...
  sharedMemory: SharedResearchMemory,
  plan?: ResearchPlan,
  extensionElements: ElementData[] = []
): Promise<AnalyzeAndDecideResult> {
...
What do you want to do? Call one tool. Respond with JSON: {"tool": "...", "args": {...}, "reasoning": "why you're doing this"}`;
  ...
  const callAI = async (useVision: boolean) => {
  ...
  const parseToolCall = (raw: string): AnalyzeAndDecideResult => {
    const d: any = JSON.parse(raw.replace(/```json\s*|```/g, ''));
    ...
    const extractedFacts: string[] = isExtract ? (args.facts || d.facts || []) : [];
    const extractedUrls: string[] = isExtract ? (args.urls || d.urls || []) : [];
    const extractedSummary: string = isExtract ? (args.summary || d.reasoning || '') : '';
...
export async function executeAction(ctx: { page: any, action: BrowserAction, onProgress: any, extensionElements: ElementData[], extensionAPI: ChromeExtensionAPI, taskId: string }): Promise<void> {
...
async function synthesizeResearch(
  ...
  if (facts.length === 0) return 'No useful sources found.';
...
export async function performSmartResearch(
    ctx: any,
    task: ParallelTask,
    taskId: string,
    aiClient: AIClient,
    ...
    bridgeServer: any,
    sharedMemory: SharedResearchMemory
): Promise<{ task: string; summary: string; sourcesVisited: number }> {
    const page = await ctx.newPage();
    const extensionAPI = new ChromeExtensionAPI();
    const workers = (ctx as any).serviceWorkers?.() || [];
    const extId = workers.find((w: any) => w.url().startsWith('chrome-extension://'))?.url()?.split('/')[2] || '';
    if (extId) extensionAPI.setPage(page, extId);
    ...
    while (steps < maxSteps) {
        ...
        const screenshot = relevanceScore < 50
          ? await page.screenshot({ type: 'jpeg', quality: 35 }).then((b: Buffer) => b.toString('base64')).catch(() => null)
          : null;

        // ONE AI call: analyze + decide simultaneously
        const { analysis, action } = await analyzeAndDecide(
          aiClient, task.task, content, screenshot, steps, maxSteps, sharedMemory, plan, extElements
        );
        ...
        if (analysis.isUseful && analysis.shouldExtract) {
            ...
            highValueUrls.forEach(d => {
                if (!sharedMemory.hasVisited(d.url)) sharedMemory.queueUrl(d.url, d.score);
            });
        ...
        if (plan.mustFind.length > 0 && sharedMemory.getFactCount() >= 3) {
            ...
            const satisfied = plan.mustFind.filter(item =>
                item.toLowerCase().split(/\s+/).filter(w => w.length > 3).some(w => allFacts.includes(w))
            );
        ...
        analysis.itemUrls.forEach((u: string, i: number) => {
            if (!sharedMemory.hasVisited(u)) sharedMemory.queueUrl(u, 85 - i * 2);
        });
        analysis.nextUrls.forEach(u => {
            if (!sharedMemory.hasVisited(u)) {
                const score = scoreUrlRelevance(u, task.task);
                if (score >= 35) sharedMemory.queueUrl(u, score);
            }
        });

        // Handle action execution — queue override only applies to navigate/goto_link/extract
        const shouldOverrideWithQueue =
          (action.action === 'navigate' || action.action === 'goto_link' || action.action === 'extract') &&
...
async function performSingleTaskResearch(
    ctx: any,
    task: ParallelTask,
    taskId: string,
    aiClient: AIClient,
...
export async function runBrowserResearch(options: BrowserUseOptions): Promise<BrowserUseResult> {
  ...
  try {
    ...
    const { bridgeServer } = await import('../../lib/extension-server');
...
export function createBrowserUseTool(aiClient: AIClient, groundingEngine?: GroundingEngine): AgentTool {
  return {
    name: 'browser_use',
    ...
    async execute(args: any, onUpdate?: any) {
...
export async function openDebugBrowser(): Promise<void> {
  const pw = require('playwright');
  const extensionPath = path.join(process.cwd(), 'public/chrome-extension');
